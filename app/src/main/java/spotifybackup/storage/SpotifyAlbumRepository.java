...
class SpotifyAlbumRepository {
    ...
    private static void setNotSimpleFields(EntityManager em, boolean storeTracks, Album apiAlbum, SpotifyAlbum album,
        ...
        album.addImages(SpotifyImageRepository.imageSetFactory(em, apiAlbum.getImages(), selection));
     ...
     * @param apiAlbum    Album object generated by the spotify-web-api.
     * @param storeTracks Limit whether tracks are stored.
     * @param selection   Limit which images are stored.
     * @return SpotifyAlbum already in the database with matching Spotify ID or new SpotifyAlbum if apiAlbum has a new
     * Spotify ID.
     */
    static SpotifyAlbum persist(EntityManager entityManager, @NonNull Album apiAlbum, boolean storeTracks,
        ...
        if (optionalAlbum.isPresent()) {
            if (!optionalAlbum.get().getIsSimplified()) return optionalAlbum.get();
                ...
                setNotSimpleFields(entityManager, storeTracks, apiAlbum, simpleAlbum, selection);
            ...
            var newAlbum = SpotifyAlbum.builder()
                    ...
                    .build();
            entityManager.persist(newAlbum);
            setNotSimpleFields(entityManager, storeTracks, apiAlbum, newAlbum, selection);
            entityManager.persist(newAlbum);
            return newAlbum;
        }
    }
