...
public class SpotifyObjectRepository {
    ...
    private final Function<AbstractModelObject, Function<EntityManagerFactory, Function<
            BiFunction<EntityManager, AbstractModelObject, Optional<? extends SpotifyObject>>,
            Optional<? extends SpotifyObject>>>> persistAbstractModel = apiObject -> factory -> persist -> {
    ...
    public Optional<SpotifyImage> persist(@NonNull Image image) {
        return (Optional<SpotifyImage>) persistAbstractModel.apply(image).apply(emf).apply(SpotifyImageRepository::persist);
    }
